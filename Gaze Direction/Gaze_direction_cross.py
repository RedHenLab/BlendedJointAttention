...
while True:
	...
	if ret:
		...
		for k, d in enumerate(dets):
			...
			x1 = shape.part(36).x-3
			y1 = shape.part(37).y-4
			x2 = shape.part(39).x+3
			y2 = shape.part(40).y+4
			split = frame[y1:y2,x1:x2]
			split = process_eye(split)
			split = filter_eye(split)
			frame[y1:y2,x1:x2]=split
			cv2.rectangle(frame,(x1,y1), (x2,y2), (0, 0, 255), 2)
			x1 = shape.part(42).x-3
			y1 = shape.part(43).y-4
			x2 = shape.part(45).x+3
			y2 = shape.part(46).y+4
			split = frame[y1:y2,x1:x2]
			split = process_eye(split)
			split = filter_eye(split)
			frame[y1:y2,x1:x2]=split
